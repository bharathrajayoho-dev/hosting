...
export default function
	...
	const validateField = (field: keyof TenantFormData, value: string) => {
		...
		switch (field) {
			...
			case 'contactName':
				...
				break;
			case 'contactPhone':
				// Contact phone is now optional, but validate format if provided
				if (value.trim() && !/^[0-9]{10}$/.test(value))
					error = 'Phone number must be 10 digits';
				break;
			case 'bankName':
				// Bank name is now optional
				break;
			case 'accountNumber':
				// Account number is now optional, but validate format if provided
				if (value.trim() && !/^[0-9]{9,18}$/.test(value)) error = 'Invalid account number';
				break;
			case 'ifscNumber':
				// IFSC code is now optional, but validate format if provided
				if (value.trim() && !/^[A-Za-z]{4}0[A-Z0-9a-z]{6}$/.test(value))
	...
	const validateForm = () => {
		...
		// Always required fields (removed emergency contact and bank details)
